...
user_data = {}
admin_ids = {}  # Хранит ID владельцев чата и их ранги
...
def admin_command(message):
    admins = [user_id for user_id, rank in user_data.items() if rank['rank'] >= 4]
    admin_list = "\n".join([str(user_id) + " (Ранг: " + titles[user_data[user_id]['rank']] + ")" for user_id in admins]) if admins else "Нет администраторов."
...
def upgrade_command(message):
    ...
    if user_id and user_id in user_data:
        ...
        bot.reply_to(message, "❌ Укажите пользователя, чий ранг необходимо повысить!")
...
# Автоматическая установка ранга для создателя чата
@bot.message_handler(content_types=['new_chat_members'])
def new_member(message):
    for new_member in message.new_chat_members:
        if new_member.id == message.chat.id:  # Проверка, является ли новый член создателем чата
