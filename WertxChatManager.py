...
def start_command(message):
    bot.send_message(message.chat.id, "👋 Привет! Я помощник чата! Вот, что я умею:\n"
                                       ...
                                       "/upw - повысить ранг пользователя (требуется ответ на сообщение)\n")
...
def cmdw_command(message):
    ...
    cmd_list += "/upw - Повысить ранг пользователя (требуется ответ на сообщение)\n"
...
def coin_command(message):
    ...
    if user_id in user_data:
        ...
        user_data[user_id] = {'coins': coins, 'messages_count': 0, 'rank': 0}
